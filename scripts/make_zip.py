...
def  main():
  ...
  installer = "\installer\SnuCrusher Installer.exe"
   
  if demo:
    installer = "\installer\SnuCrusher Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\SnuCrusher manual.pdf" 
  ]
  ...
  ZIPNAME = "SnuCrusher-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "SnuCrusher-v" + FULLVERSIONSTR + "-win-demo.zip"
